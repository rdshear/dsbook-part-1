...
```{r labels-better-than-legends, cache=FALSE}
...
```{r ridge-plot, message=FALSE, cache=FALSE}
library(ggridges)
p <- gapminder |> 
...
```{r ecological-fallacy-averages, echo=FALSE, cache=FALSE}
library(ggrepel)
gapminder <- gapminder |> 
  mutate(group = case_when(
...
```{r ecological-fallacy-all-data, echo=FALSE, cache=FALSE}
library(ggrepel)
